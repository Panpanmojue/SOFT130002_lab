...
function testTime(){
    let count = 0;
    let value = 1;
    let stop = setInterval(change,5000);
    setInterval(f,1000);
    function change() {
        count++;
        value = value*2;
...
function testMail(telephone,mail) {
    let telephoneNum = /^1[0-9]{10}$/;
    let mailNum = /^[A-Za-zd]+([-_.][A-Za-zd]+)*@([A-Za-zd]+[-.])+[A-Za-zd]{2,4}$ /g;
    let teleResult = telephoneNum.test(telephone);
    let mailResult = mailNum.test(mail);
    let out = "The telephone is " + (teleResult?"right":"wrong") + ", and the mail is " + (mailResult?"right":"wrong") + "!";
    console.log(out);
}
testMail("19121757506","1015068680@qq.com");
...
function testRedundancy(str) {
    const repeat = /\b([a-z]+) \1\b/ig;
    let array = str.split(" ");
    let check = [];
    let result = [];
    let num;
    for (let i = 0;i < array.length - 1;i++){
        check[i] = array[i] + " " + array[i+1];
        if (check[i].match(repeat)){
            result[num] = check[i];
            num++;
        }
    }
